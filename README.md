...
Background across **EdTech, HRTech, FinTech, and SecurityTech**.
...
`voice AI` `transcription` `LLM extraction` `schema-configurable` `evals` `TypeScript`

### 🧾 [BookkeepGenie](https://github.com/AndrewLocke-pm/BookKeepGenie)
AI-first bookkeeping for South African small businesses with a 5-layer transaction enforcement system — rules-first classification, LLM fallback for edge cases, a human confirmation modal, server-side enforcement, and database constraints as the last line of defence. Includes a 40-test automated suite with a pre-deploy gate.
`LLM APIs` `AI enforcement patterns` `automated testing` `financial domain`

### 🎯 [StrikeCRM](https://github.com/AndrewLocke-pm/strikecrm)
A voice-to-data CRM demo built on the Transcript Insights v1.0 engine — turns sales calls into structured deal updates, with a deal state machine, multi-tenant org isolation, RBAC, and an immutable audit log.
`LLM extraction` `deal state machine` `multi-tenant` `RBAC` `TypeScript`
